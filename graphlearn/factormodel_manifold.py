...
class FactorModel(RiemannianSubmanifold):
    ...
    def __init__(self, n_features, rank):
        ...
        dimension = int(self.n_features*self.rank - self.rank *
                        (self.rank-3)//2 + self.n_features)
    ...
    def inner_product(self, point, tangent_vector_a, tangent_vector_b):
        inner_st = np.tensordot(tangent_vector_a[0], (np.eye(
            self.n_features) - 0.5 * point[0] @ point[0].T) @ tangent_vector_b[0], axes=point[0].ndim)
        #
        # some replacement needed to optimize stuff a bit !
        L = la.cholesky(point[1])
        ...
        inner_dpd = np.tensordot(
            tangent_vector_a[2] / point[2], tangent_vector_b[2] / point[2], axes=point[2].ndim)
    ...
    def _projection_horizontal(self, point, vector):
        ...
        tmp1 = -3*np.eye(self.rank**2) + 2 * \
            (np.kron(point[1], iS) + np.kron(iS, point[1]))
    ...
    def euclidean_to_riemannian_gradient(self, point, euclidean_gradient):
        return _ProductTangentVector([euclidean_gradient[0] - point[0] @ euclidean_gradient[0].T @ point[0],
                                      point[1] @ sym(euclidean_gradient[1]
                                                     ) @ point[1],
    ...
    def retraction(self, point, tangent_vector):
        ...
        return [v @ w,
                sym(point[1] + tangent_vector[1] + 0.5 * tangent_vector[1]
                    @ la.solve(point[1], tangent_vector[1])),
...
class FactorModel2SPD(MappingBetweenManifolds):
    ...
    def differential(self, point, tangent_vector):
        return tangent_vector[0] @ point[1] @ point[0].T + point[0] @ point[1] @ tangent_vector[0].T + \
            point[0] @ tangent_vector[1] @ point[0].T + \
            np.diag(tangent_vector[2])
